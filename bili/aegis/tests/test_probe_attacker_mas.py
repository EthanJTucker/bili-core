...
class _StubObserver:
    ...
    def __call__(
        ...
        return (
            {
                "observation_summary": "stub observation",
...
def test_run_session_terminates_on_success_verdict():
    ...
    attacker = _build_attacker(
        evaluator=_StubEvaluator(score=3, verdict=TurnVerdict.SUCCESS)
    )
    sess = _session()
...
def test_run_session_terminates_on_budget_exceeded():
    ...
    attacker = _build_attacker(
        evaluator=_StubEvaluator(score=1, verdict=TurnVerdict.PARTIAL_PROGRESS)
    )
    sess = _session()
    budget = BudgetState(
        max_turns=2,
...
def test_run_session_passes_crafted_payload_to_victim():
    ...
    victim = _StubVictimExecutor()
    crafter = _StubCrafter(payload="UNIQUE_PAYLOAD_MARKER")
    attacker = _build_attacker(
        crafter=crafter,
        evaluator=_StubEvaluator(score=1, verdict=TurnVerdict.PARTIAL_PROGRESS),
    )
...
def test_run_session_per_turn_token_breakdown_matches_node_returns():
    ...
    attacker = _build_attacker(
        crafter=_StubCrafter(tokens=(20, 10)),  # 30 total
...
def test_final_outcome_tier3_score_is_max_across_turns():
    ...
    attacker = _build_attacker(evaluator=_VaryEvaluator())
...
def test_final_outcome_aggregates_per_turn_tokens():
    ...
    attacker = _build_attacker(
        crafter=_StubCrafter(tokens=(20, 10)),
        evaluator=_StubEvaluator(
            score=1, verdict=TurnVerdict.PARTIAL_PROGRESS, tokens=(15, 8)
        ),
    )
